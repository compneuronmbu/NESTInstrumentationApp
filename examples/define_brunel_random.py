...
"""
Definition of spatially extended Brunel network with randomly placed nodes.

This module provides layer and projections declarations suitable for
use with the NEST Topology Module.

It defines a Brunel-style network with neurons places on a random grid.
...
def make_layers():
    ...
    layers = [('PoissonGenerator', {'rows': 1, 'columns': 1,
                                    'elements': 'stimulator'}),
              ('Excitatory', {'edge_wrap': True,
                              'elements': 'excitatory',
                              'positions': [[uniform(-0.5, 0.5), uniform(-0.5, 0.5)]
                                            for _ in range(P.NE)]}),
              ('Inhibitory', {'edge_wrap': True,
                              'elements': 'inhibitory',
                              'positions': [[uniform(-0.5, 0.5), uniform(-0.5, 0.5)]
                                            for _ in range(P.NI)]}),
